  ...
  match free_list {
      ...
      FreeList::Node(base, free_size, rest) => {
          if size == free_size {
              ...
              Some((FreeList::Node(base + size, free_size - base, rest), base))
          }
          else {
              match malloc(*rest, size) {
                  None => None,
                  Some((rest2, base2)) =>
                    Some((FreeList::Node(base, free_size, Box::new(rest2)),
                        base2))
              }
          }
      }
...
fn eval_rec(st:  &mut State, env: &Env, instr: &Instr) -> R {
    match instr {
        ...
        Instr::Load(r, v, rest) => {
            ...
            if ptr >= st.heap.len() {
                return Err(
                    Error::Runtime(format!("{} = *{:?} invalid address {}",
                r, v, ptr)));
            }
        ...
        Instr::Goto(v) => {
            ...
            match env.instructions.get(&code_ptr) {
                ...
                Option::None => Err(Error::Runtime(
                    format!("goto({}) invalid code address", code_ptr)))
            }
        },
        Instr::Exit(v) => {
            Result::Ok(eval_val(&st.registers, v))
        },
        Instr::Abort() => {
            Result::Ok(0)
        },
        ...
        Instr::Malloc(r, v, rest) => {
            ...
            let (free_list2, ptr) = try!(malloc(nil_list, n).ok_or(Error::Runtime("malloc OOM".to_string())));
        ...
        Instr::Free(r, rest) => {
            let ptr = st.registers[*r] as usize;
            let mut nil_list = FreeList::Nil;
            let size = *try!(st.alloc_blocks.get(&ptr).ok_or(Error::Runtime("free bad ptr".to_string())));
            std::mem::swap(&mut st.free_list, &mut nil_list);
...
pub fn eval(heap_size : usize, num_registers : usize,
  blocks: HashMap<i32, Instr>) -> R {
    let mut st = State {
    ...
    let env = Env { instructions: blocks };
    env.instructions.get(&0)
